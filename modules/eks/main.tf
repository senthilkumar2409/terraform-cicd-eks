...
################################################################
# IAM role for developer to access a EKS cluster
################################################################
...
#####################################################
resource "aws_iam_group_membership" "iam_user_attachment" {
  name = "iam-user-assignment"
...
resource "aws_eks_access_entry" "eks_cluster" {
  principal_arn = "arn:aws:iam::975049977826:user/kube-developer"
  cluster_name = aws_eks_cluster.cluster.name
  kubernetes_groups = [ "developers" ] #It is the group name which we specify in a rolebinding.yaml manifest file
  type = "STANDARD"
}
